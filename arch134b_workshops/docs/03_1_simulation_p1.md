...
Try recording your simulation EUI with your building vintage of choice. When you change the vintage, ensure these construction sets are reflected in the panels attatched to your main workflow. Compare the EUI side by side. Do your results align with your expectations? 

```{image} ../_static/sim1/sim1_14.png
:width: 100%
:align: center
```
Hover over the HB End Use Intensity (EUI)'s ip input to view what units of measure are available. Without a Boolean Toggle, it defaults to False, which produces results in kWh/m^2. To change to Imperial System Units (IP), set a Boolean Toggle from the ip to True. 
